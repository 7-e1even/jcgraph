...
/**
 * Abstract JVM frame simulator. A taint-tracking method visitor subclasses this
 * and overrides {@code visitMethodInsn} to define propagation; this base only
 * maintains the shape of the operand stack / locals (push/pop per opcode) with a
 * per-slot taint {@link Set}.
 *
 * @param <T> taint marker type
 */
                ...
                for (int i = 0; i < argTypes.length; i++) {
                    ...
                    if (argType.getSize() > 0) {
                        ...
                        for (int j = 0; j < argType.getSize() - 1; j++) {
                            merged.addAll(operandStack.pop());
                        }
                        merged.addAll(operandStack.pop());
